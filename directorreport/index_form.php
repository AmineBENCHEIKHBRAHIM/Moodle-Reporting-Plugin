 ...
 */
 ...
 class directorreport_form extends moodleform {
 ...
 $mform->addElement('submit', 'save', get_string('display','report_directorreport'),'align="right"');
 }
 ...
 if($data['course']=='0')
 {
 $errors['course']=get_string('error_invalidcourse','report_directorreport');
 }
 //added to compare last access date and current date
 if($data['lastaccesseddate']>$date['currentdate'])
 {
 $errors['lastaccesseddate']=get_string('error_invaliddate','report_directorreport');
 }
 //added to compare current date with the system date
 if($data['currentdate']>time(date("d-m-Y")))
 {
 $errors['currentdate']=get_string('error_invalidcurrentdate','report_directorreport');
 }
 //added to check the last accessed date is not equal to null/zero
 if($data['lastaccesseddate']=='0')
 {
 $errors['lastaccesseddate']=get_string('error_nolastaccessdate','report_directorreport');
 }
